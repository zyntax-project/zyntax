...
pub(crate) const BOOL: i64 = 1;
pub(crate) const INT: i64 = 2;
pub(crate) const UINT: i64 = 3;
pub(crate) const FLOAT: i64 = 4;
pub(crate) const STR: i64 = 5;
pub(crate) const CUSTOM: i64 = 255;
...
/// Asked only of two custom-category boxes, and out of line, so the
/// number paths of the operators it guards stay as short as they were.
fn keyed_set_pair(a: Expr, b: Expr) -> Expr {
    call("zb_any_keyed_set_pair", vec![a, b], boolean())
}
...
pub(crate) fn declarations(policy: &Policy, list_type: TypeId) -> Vec<Decl> {
    ...
    // Two custom boxes that are sets, at least one of a shape the
    // frontend registered.
    {
        let set_of = |x: Expr| or(eq(kind(x.clone()), int(SET_TAG >> 8)), is_keyed_set(x));
        d.push(define_cold(
            "zb_any_keyed_set_pair",
            &[&a, &b],
            boolean(),
            vec![
                when(
                    not(or(is_keyed_set(a.e()), is_keyed_set(b.e()))),
                    vec![ret(bool(false))],
                ),
                ret(and(set_of(a.e()), set_of(b.e()))),
            ],
        ));
    }
    ...
    d.push(define(
        ...
        vec![
            ...
            when(
                ...
                vec![
                    // A dict or set of a shape compares through its hooks,
                    // with a dict or set of any shape, before the kinds are
                    // told apart.
                    when(
                        or(is_keyed(a.e()), is_keyed(b.e())),
                        vec![ret(call(
                            "zb_hook_shaped_eq",
                            vec![a.e(), b.e()],
                            boolean(),
    ...
    d.push(define(
        ...
        vec![
            ...
            // A frozen set of a shape hashes through its hooks; a dict of
            // one raises there.
            when(
                is_keyed(x.e()),
                vec![ret(call("zb_hook_shaped_hash", vec![x.e()], i64()))],
            ),
    ...
    d.push(define(
        ...
        vec![
            when(
                is_keyed(x.e()),
                vec![
                    expr(call("zb_hook_shaped_delitem", vec![x.e(), i.e()], unit())),
                    ret_void(),
                ],
            ),
            when(
