...
/// the baseline threshold next. A request made while the thread is in
...
impl CompileQueue {
    ...
    fn wait(&self) {
        let guard = self.compile.lock().unwrap();
        if !self.promote.lock().unwrap().is_empty() || !guard.is_empty() {
            return;
        }
        if self.stop.load(std::sync::atomic::Ordering::Acquire) {
            return;
        }
        // Timed, so a stop raised between the checks and the wait is
        // seen without a notification.
        let _ = self
            .ready
...
thread_local! {
    /// Set on the warm-up thread: a request raised there runs inline,
    /// since the thread that would take it is this one.
    static ON_WARM_UP: std::cell::Cell<bool> = const { std::cell::Cell::new(false) };
}
...
pub struct TieredBackend {
    ...
    /// What the warm-up thread compiles next: the requests of frames
...
impl TieredBackend {
    ...
    pub fn compile_module_lazily(
        ...
        // Every bead now exists, so the handlers can capture them. The
        // lazy compiler first: the requester hands interpreted requests
        // to its queue. The compiler installed covers every module so
        // far: a program that compiles a chunk of itself still calls its
        // earlier functions for the first time afterwards.
    ...
    /// The body the interpreter runs for each lazy function: the
    /// optimised one once its first compile has made it, so a frame can
    /// move to the code compiled from it; before that, the module's as
    /// lowered with its releases placed, since optimising a body before
    /// its first run cost every function the whole pipeline, run once or
    /// not, and a frame that warms leaves into a resume point optimised
    /// on its own. A body that arrived optimised gets what little the
    /// optimiser does to it. `None` leaves the module's body to the
    /// interpreter.
    pub fn interpreter_body_source(
        ...
        let make_interp_body = self.make_interp_body.clone();
    ...
    /// The module function behind a code address, for the interpreter's
    /// calls through pointers: a body the ground tier published, a
    /// stub, or the code a tier above installed in a bead.
    pub fn interpreter_address_source(&self) -> Box<dyn Fn(usize) -> Option<HirId> + Send + Sync> {
        let cranelift = Arc::clone(&self.cranelift);
        let beads: Vec<(HirId, TieredBound)> = self
            .functions
            .iter()
            .map(|(id, e)| (*id, e.bound.clone()))
            .collect();
        Box::new(move |addr: usize| {
            if let Some(id) = cranelift.with_lock(|be| be.function_at(addr)) {
                return Some(id);
            }
            beads
                .iter()
                .find(|(_, bound)| bound.bead().compiled().is_some_and(|p| p as usize == addr))
                .map(|(id, _)| *id)
        })
    }
    ...
    fn install_promotion_requester(&self) {
        ...
        let adapter = Arc::clone(&self.adapter);
        let cranelift = Arc::clone(&self.cranelift);
        let optimize_extra = self.optimize_extra.clone();
        #[cfg(feature = "llvm-backend")]
        let llvm = self.llvm.as_ref().map(Arc::clone);
        // Regions outlined for a bead's frames and not yet compiled at
        // the tier above, by bead. The promotion that gets there first
        // takes them, whichever frame asked for it: the one in the region
        // asks through the region's own probes.
        #[allow(clippy::type_complexity)]
        let outlined_regions: Arc<Mutex<HashMap<u64, Vec<(HirId, Arc<HirFunction>)>>>> =
            Arc::new(Mutex::new(HashMap::new()));
        // Resume points asked for after a bead's promotion (see
        // [`LateResumePoints`]), and the regions promoted with it, which
        // a late request from a frame in one names by body tag.
        #[cfg(feature = "llvm-backend")]
        let late = Arc::new(Mutex::new(LateResumePoints::default()));
        #[cfg(feature = "llvm-backend")]
        #[allow(clippy::type_complexity)]
        let promoted_regions: Arc<Mutex<HashMap<u64, Vec<(HirId, Arc<HirFunction>)>>>> =
            Arc::new(Mutex::new(HashMap::new()));
        ...
        osr::set_promotion_requester(move |bead_id, from| {
            ...
            // request costs the running loop only the submission. The
            // regions frames run come first: a frame is in one now, the
            // body serves the calls after.
            let submitted = adapter.force_promote(bound, tier_idx, move |bead| {
                let regions = outlined_regions
                    .lock()
                    .unwrap()
                    .remove(&bead_id)
                    .unwrap_or_default();
...
#[cfg(feature = "llvm-backend")]
#[derive(Default)]
struct LateResumePoints {
    queued: HashSet<u64>,
    installed: HashSet<u64>,
    claimed: HashSet<(u64, u64)>,
}

#[cfg(feature = "llvm-backend")]
impl LateResumePoints {
    /// Whether the caller makes the resume point at `site` now: the bead
    /// is installed and nobody else has claimed the site.
    fn claim_if_installed(&mut self, bead_id: u64, site: u64) -> bool {
        self.installed.contains(&bead_id) && self.claimed.insert((bead_id, site))
    }

    fn release(&mut self, bead_id: u64, site: u64) {
    ...
    /// caller.
    fn installed(&mut self, bead_id: u64) -> Vec<u64> {
        self.installed.insert(bead_id);
        osr::sites_asked(bead_id)
            .into_iter()
...
/// The bodies a late resume point of `bead_id` can be made from: the
/// function's own, as the optimizing tier compiled it, and the regions
/// outlined for its frames. Each as `(function id, body)`.
#[cfg(feature = "llvm-backend")]
fn late_resume_bodies(
    optimized: Option<Arc<HirFunction>>,
    module_body: Option<&HirFunction>,
    regions: &[(HirId, Arc<HirFunction>)],
    func_id: HirId,
) -> Vec<(HirId, Arc<HirFunction>)> {
    let mut bodies: Vec<(HirId, Arc<HirFunction>)> = regions.to_vec();
    match (optimized, module_body) {
        (Some(f), _) => bodies.push((func_id, f)),
        (None, Some(f)) => bodies.push((func_id, Arc::new(f.clone()))),
        (None, None) => {}
    }
    bodies
}

/// The optimizing tier's resume point at `site` for `bead_id`, made
/// from whichever of `bodies` the site's tag names, and published.
#[cfg(feature = "llvm-backend")]
fn publish_late_resume_point(
    llvm: &Arc<ZyntaxLlvmBackend>,
    bodies: &[(HirId, Arc<HirFunction>)],
    module_arc: &Arc<HirModule>,
    bead_id: u64,
    site: u64,
) {
    ...
    let made = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        llvm.resume_points(&def, std::iter::once(site).collect())
    }))
    .unwrap_or_default();
    if osr::osr_trace_enabled() {
        for (site, code) in &made {
            eprintln!(
                "[osr] {} site=0x{site:x}: late resume point (llvm) -> {code:?}",
...
/// [`publish_baseline_resume_points`] for a frame on the body as
/// lowered: the region from the header is outlined, optimised with
...
fn publish_outlined_resume_point(
                ...
                cranelift
                    ...
                    .map(|out| {
                        let sites: Vec<(u64, usize)> = out
                            ...
                            .collect();
                        (sites, out.region_id, out.region)
                    })
            })
            .map(|h| h.join().unwrap_or_default())
            .unwrap_or_default()
    });
    let Some((sites, region_id, region)) = outlined else {
        if osr::osr_trace_enabled() {
            eprintln!(
                "[osr] {}: region not outlined, resume point from the body",
                func_arc.name.resolve_global().unwrap_or_default()
            );
        }
    ...
    for (site, code) in sites {
        ...
        if !code.is_null() && osr::helper_for(bead_id, site).is_null() {
            if osr::osr_trace_enabled() {
                eprintln!(
                    "[osr] {} site=0x{site:x}: outlined resume point",
                    func_arc.name.resolve_global().unwrap_or_default()
                );
            }
...
/// region compiled there, its helpers published under the bead at the
/// region's own sites, which the probes of its baseline code ask at. The
/// frame in the region moves up mid-loop, as a frame in the body does.
/// The region's cell then enters the code above too.
#[cfg(feature = "llvm-backend")]
fn promote_outlined_region(
    llvm: &Arc<ZyntaxLlvmBackend>,
    bead: &Arc<Bead>,
    region_id: HirId,
    region: &Arc<HirFunction>,
    module_arc: &Arc<HirModule>,
    bead_id: u64,
    cranelift: &Arc<ZyntaxCraneliftBackend>,
    verbosity: u8,
    ...
    let tier_idx = OptimizationTier::Optimized.index();
    let name = region.name.resolve_global().unwrap_or_default();
    if verbosity >= 1 || osr::osr_trace_enabled() {
        eprintln!(
            "[TieredBackend] Recompiling {region_id:?} ({name}) at tier {tier_idx} ({:?})",
    ...
    match compiled {
        Ok(entry) if !entry.is_null() => {
            let key = cranelift.with_lock(|be| be.reload_key());
            crate::reload::set_call_target(key, region_id, entry as usize);
        }
        Ok(_) => {}
        Err(e) => {
