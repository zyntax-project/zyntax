...
//! finalizer runner, the compaction of weak tables after a
//! collection, and `warn`.
...
pub(super) fn declarations(t: &Types) -> Vec<Decl> {
    ...
    let mut d = vec![
        ...
        extern_fn("zl_gc_next_dirty", &[], i64(), Some("$Lua$gc_next_dirty")),
        extern_fn(
            "zl_warn_piece",
            ...
            while_(
                ...
                vec![
                    ...
                    when(
                        not(is_nil(y.e())),
                        vec![
                            expr(call("zl_call_1", vec![y.e(), box_table(tb.e())], any())),
                            when(
                                not(is_nil(pending())),
                                vec![
    ...
    // After a collection the program asked for: the weak tables it
    // left tombstones in are compacted, and the finalizers it queued
    // run.
    d.push(define(
        "zl_gc_after",
        &[],
        unit(),
        vec![
            p.decl(call("zl_gc_next_dirty", vec![], i64())),
            while_(
                ne(p.e(), int(0)),
                vec![
                    expr(call(
                        "zl_gc_compact",
                        vec![cast(p.e(), table.clone())],
                        unit(),
                    )),
                    p.set(call("zl_gc_next_dirty", vec![], i64())),
                ],
            ),
    ...
    body.extend([
        ...
        when(
            ...
            vec![
                expr(call("zl_gc_after", vec![], unit())),
                when(chosen("step"), vec![ret(box_bool(bool(true)))]),
                ret(box_i64(int(0))),
            ],
        ),
        when(
