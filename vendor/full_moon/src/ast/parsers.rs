...
pub fn parse_block(state: &mut ParserState) -> ParserResult<ast::Block> {
    ...
    loop {
        // Lua 5.2 and later allow an empty statement wherever a
        // statement may stand; each is a lone `;` that belongs to no
        // node.
        while state.consume_if(Symbol::Semicolon).is_some() {}
